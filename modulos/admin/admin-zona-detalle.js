...
async function cargarDetalleZF(id) {
  ...
  try {
    ...
    document.getElementById('badge-zf-provincia').innerHTML = `<span class="badge-estado badge-estado--recomendada" style="font-size:1rem; padding: 0.4rem 0.85rem;">📍 ${provinciaCanton}</span>`;
      ...
      <div class="card" style="padding: 1rem;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.75rem;">
          <h3 style="font-size: 1.15rem; margin: 0; display: flex; align-items: center; gap: 0.5rem;">
            🗺️ Ubicación Geográfica en Mapa (Lat: ${lat}, Lng: ${lng})
          </h3>
    ...
    setTimeout(() => {
      ...
      if (mapDiv && window.L) {
        ...
        marker.bindPopup(`<b>📍 ${zf.nombre}</b><br>${direccionCompleta}`).openPopup();
      }
    }, 50);
    ...
    contenedor.innerHTML = `
      <div class="card" style="text-align: center; color: var(--estado-rechazada-texto);">
        ⚠️ No se encontró la Zona Franca (ID: ${id}): ${err.message}
      </div>
    `;
  }
}
